      ...
      <div className="rounded-xl border border-stone-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {report.topTraits.map((trait) => (
            <span
              key={trait}
              className="rounded-full bg-[#17808d]/10 px-3 py-1 text-sm font-medium text-[#4a2373]"
            >
              {trait}
            </span>
          ))}
          ...
          {topClusters.map(({ cluster, score }) => (
            <div key={cluster.id}>
              <div className="flex items-baseline justify-between">
                ...
                <span className="text-sm font-bold text-[#7a3e9d]">{score}%</span>
              </div>
          ...
          {report.families.map((family, index) => (
            <div key={family.id} className="flex items-center gap-2 text-sm">
              ...
              <div className="h-2 w-28 overflow-hidden rounded-full bg-stone-100">
                <div
                  className="h-full rounded-full bg-[#7a3e9d]"
