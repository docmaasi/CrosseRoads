...
export function PlannerHero({ phaseProgress, totalDone, totalItems }) {
  return (
    <div className="relative overflow-hidden">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -top-20 right-[-10%] h-64 w-64 rounded-full bg-[#2e7d8c]/10 blur-3xl"
      />
      <div className="relative mx-auto max-w-3xl px-4 pb-4 pt-12 text-center">
        <span className="inline-flex items-center gap-2 rounded-full border border-[#2e7d8c]/25 bg-white/80 px-4 py-1.5 text-sm text-[#1e4d5c] shadow-sm">
          <GraduationCap className="h-4 w-4 text-[#2e7d8c]" aria-hidden="true" />
          College Roadmap
        </span>
          ...
          {PHASES.map((phase, index) => {
            ...
            return (
              ...
              >
                ...
                <p className="mt-0.5 font-serif font-bold leading-snug text-[#1e4d5c]">
                  {phase.name}
                </p>
